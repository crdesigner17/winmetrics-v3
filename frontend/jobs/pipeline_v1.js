...
async function processarFixture(fix, liga, teamCache, fixtureCache) {
  ...
  const fixtureDate = fix.fixture?.date || null;
  const hora   = fixtureDate
    ? new Date(fixtureDate).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Sao_Paulo' })
    : '00:00';
...
async function salvarNoSupabase(jogo, matchDateISO) {
  const officialMatchDate = jogo.match_date || matchDateISO;
  // Upsert fixture
  ...
  if (fixtureErr) {
    LOG.error(`Fixture upsert error (${jogo.fixture_id}): ${fixtureErr.message}`);
    return false;
  }

  if (!fixtureRow?.fixture_id) {
    LOG.error(`Fixture upsert did not return fixture_id=${jogo.fixture_id}; snapshot skipped.`);
    return false;
  }
